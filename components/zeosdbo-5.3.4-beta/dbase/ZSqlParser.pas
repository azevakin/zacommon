...
uses Classes, DB, ZSqlTypes, ZSqlItems {$IFDEF VER100}, DbTables {$ENDIF}
  ...
  {$IFDEF VER150ABOVE}{$IFNDEF VERCLX}, Variants{$ENDIF}{$ENDIF};
...
type
  ...
  private
    ...
    function  ReplaceAliasesWithFieldNames(SqlString:string):string;
  protected
...
begin
  ...
  Dataset.Close;
  if Assigned(Dataset) and Dataset.ParamCheck then begin
    ...
    try
      ...
      { Update field defs }
      if not Dataset.DefaultFields then begin
...
//Changes suggested by Dmitry Chernikov on 11/13/2001
//comment: To generate sql text from filter text we need to replace
//         aliases with fully qualified physical field names
...
begin
  with DataSet as TZDataset do
    if (doSqlFilter in Options) and Filtered and (Trim(Filter) <> '') then
      //replace aliases with fully qualified field names
      Result := ReplaceAliasesWithFieldNames(Filter)
  else Result := '';
end;

//Private funtion suggested by Dmitry Chernikov on 11/13/2001
function TSqlParser.ReplaceAliasesWithFieldNames(SqlString:string):string;
var
    I: Integer;
...
begin
   ...
   for I:=0 to SqlFields.Count-1 do
      begin
      FieldDesc :=SqlFields.Items[I];
      with (FieldDesc^) do
         if (Alias <> Field) then
            TempStr:=StringReplace(TempStr,Alias,Table+'.'+Field,[rfReplaceAll]);
      end;
   Result:=TempStr;
end;
...
var
  Dataset: TZDataset;
  Select, From, StrWhere, TableOid: string;
  TableOidLen: Integer;
begin
  Dataset := TZDataset(Self.Dataset);
  FText := FSql.Text;
  ...
  begin
    ...
    begin
      //Changes suggested by Dmitry Chernikov on 11/13/2001
      //Positions of WHERE, ORDERBY changes after modify sql string.
      TableOid :=' '+Aliases[0]+'.oid,';
      TableOidLen :=Length(TableOid);
      Insert(TableOid, FText, FSelectStartPos);
      FUsedRowid := True;
      //correct positions after insert
      FWhereStartPos :=FWhereStartPos + TableOidLen;
      FWherePos :=FWherePos + TableOidLen;
      FOrderPos :=FOrderPos + TableOidLen;
    end;
  end
...
begin
  ...
  Tables.Clear;
  Aliases.Clear;
// Speedpatch itSDS  SqlFields.Clear;
// 19.05.2002        SqlIndices.Clear;
...
{ Fill all fields of the query tables }
  ExtractTables(From, Tables, Aliases);
  if sqlFields.Count = 0 then   // itSDS 19.5.2002
  ...
  while True do begin
    ...
    if (Dataset.DatabaseType = dtMySql) and ((Temp = 'STRAIGNT_JOIN')
      ...
      Continue;
    if (Dataset.DatabaseType = dtMsSql) and (Temp = 'TOP') then begin
      Token := StrTok(Select, ' '#13#10);
      Continue;
    end;
    Select := Token + Select;
