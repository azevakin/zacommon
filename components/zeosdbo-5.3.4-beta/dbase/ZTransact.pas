...
uses
  {$IFNDEF LINUX} ExtCtrls, {$ELSE}
  {$IFNDEF NO_GUI} QExtCtrls, {$ELSE}ZLTimer,{$ENDIF}
  {$ENDIF}
...
type
  ...
  public
    ...
    function ExecSql(Sql: WideString; comp : TComponent): LongInt; virtual;
...
{ Execute a query }
function TZTransact.ExecSql(Sql: WideString; comp : TComponent): LongInt;
var
{$IFNDEF NO_GUI}
  OldCursor: TCursor;
{$ENDIF}
...
{$ENDIF}
    ...
    begin
      ...
      Recovery(False);
      DatabaseError(Format('%s: %s', [comp.Name, Error]));
    end else
      DoDataChange(Sql);
