...
uses
  FR_Fmted, FR_PrDlg, FR_Prntr, FR_Utils, FR_Const, FR_passw
...
{    Format: Integer;
  ...
  WriteStr(' Highlight.Condition="' + StrToXML(HighlightStr) +
    ...
    '" LineSpacing="' + IntToStr(LineSpacing) +
...
const
  BandNames: array[TfrBandType] of String =
    ('TfrReportTitle', 'TfrReportSummary', 'TfrPageHeader', 'TfrPageFooter',
     'TfrHeader', 'TfrMasterData', 'TfrFooter',
     'TfrHeader', 'TfrDetailData', 'TfrFooter',
     'TfrHeader', 'TfrSubDetailData', 'TfrFooter',
     '', 'TfrColumnHeader', 'TfrColumnFooter',
     'TfrGroupHeader', 'TfrGroupFooter',
     '', '', '',
     'TfrChild', '');
...
begin
  ...
  if BandType in [btMasterData, btDetailData, btSubDetailData] then
    WriteStr(' DataSet="' + StrToXML(DataSet) + '"');
  ...
  begin
    ...
    begin
      ...
      if not (Typ in [btPageFooter, btColumnFooter, btOverlay, btNone]) then
        ...
        begin
         if not ( (DoCalcHeight>Parent.CurBottomY-Parent.TopMargin) and (Parent.CurY=Parent.TopMargin)) then
          begin
          Result := True;
          if Parent.Skip then
            Exit else
...
begin
 ...
 //   for j := 0 to Copies - 1 do
      for i := 0 to EMFPages.Count - 1 do
        if (pgList.Count = 0) or (pgList.IndexOf(IntToStr(i + 1)) <> -1)
then
        begin
          PrintPage(i);
          if Terminated then
