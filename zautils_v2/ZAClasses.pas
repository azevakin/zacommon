...
uses Classes, DB, ZConnection, ZDataset;
...
type
  ...
  public
    ...
    constructor Create(const AOwner: TComponent; const AConnection: TZConnection); reintroduce; overload; virtual;
  end;
...
constructor TZeosClass.Create(const AOwner: TComponent; const AConnection: TZConnection);
begin
  inherited Create(AOwner);
  Connection := AConnection;
end;

function TZeosClass.ExecQuery(const SQL: string;
  const Args: array of const): TZQuery;
begin
  Result := NewQuery;
...
function TZeosClass.ExecQuery(const SQL: string): TZQuery;
begin
  Result := ExecQuery(SQL, []);
end;

function TZeosClass.NewQuery: TZQuery;
begin
  Result := TZQuery.Create(nil);
  Result.Connection := Connection;
end;

function TZeosClass.OpenQuery(const SQL: string;
  const Args: array of const): TZQuery;
begin
  Result := NewQuery;
  Result.Sql.Add(Format(SQL, Args));
...
function TZeosClass.OpenQuery(const SQL: string): TZQuery;
begin
